...
def handle_stor(cmd: Command, data: dict = None, processing_node=None) -> tuple[int, str, dict]:
    """ Maneja el comando STOR <filename> para subir un archivo al servidor. """
    ...
    if not data_nodes:
        logger.warning("No DataNodes available for STOR command")
    ...
    for data_node in data_nodes:
        try:
            msg = Message(type=MessageType.DATA_STORE_FILE_MSG, src=processing_node.ip, dst=data_node["ip"], payload={"user": session.get_username(), "cwd": session.get_cwd(), "path": filename, "session_id": session.get_session_id(), "chunk_size": 65536})
            ...
            logger.warning("Failed to contact DataNode (%s) for STOR: %s", data_node["ip"], e)
            continue
    ...
    if response.metadata.get("status") != "OK":
        error_msg = response.metadata.get("message", "Failed to store file.")
        return 550, error_msg, session.to_json()

    return 212, f"File '{filename}' stored successfully.", session.to_json()
