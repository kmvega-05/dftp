...
def handle_list(cmd: Command, data: dict = None, processing_node=None) -> tuple[int, str, dict]:
    ...
    # LIST solo acepta 0 o 1 argumento
    if cmd.arg_count() > 1:
    ...
    for data_node in data_nodes:
        try:
            msg = Message(type=MessageType.DATA_LIST, src=processing_node.ip, dst=data_node["ip"], payload={"user": session.get_username(), "cwd": session.get_cwd(), "path": path, "session_id": session.get_session_id(), "detailed": True})
