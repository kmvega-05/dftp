...
logger = logging.getLogger("dftp.processing.handlers.mkd")

def handle_mkd(cmd: Command, data: dict = None, processing_node=None) -> tuple[int, str, dict]:
    """Maneja el comando MKD <directory>."""

    if not cmd.require_args(1):
        return 501, "Syntax error in parameters. Usage: MKD <directory>", None
    ...
    if not data_nodes:
        logger.warning("No DataNodes available for MKD command")
        return 451, "Requested action aborted. File system unavailable.", None

    response = None
    for data_node in data_nodes:
        try:
            msg = Message(MessageType.DATA_MKD, processing_node.ip, data_node["ip"], payload={"user": session.get_username(), "cwd": session.get_cwd(), "path": path})
    ...
    if response.metadata.get("status") != "OK":
        return 550, response.metadata.get("message", "Failed to create directory."), None

    return 257, f'Directory "{path}" created successfully.', None
